    ...
    Element
{
    ...
    {
        "\t<cim:DynamicsFunctionBlock rdf:%s=\"%s\">\n%s\t</cim:DynamicsFunctionBlock>".format (if (about) "about" else "ID", id, export_fields)
    }
}
    ...
    Element
{
    ...
    override def export: String =
    {
        "\t<cim:RotatingMachineDynamics rdf:%s=\"%s\">\n%s\t</cim:RotatingMachineDynamics>".format (if (about) "about" else "ID", id, export_fields)
    }
}
