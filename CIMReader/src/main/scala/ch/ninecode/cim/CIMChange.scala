        ...
        Serializable
{
    ...
    def lambda (modification: (String, Iterable[(String, String, Element)])): List[(String, Option[(Option[Element], Option[Element])])] =
    {
        ...
        List[Option[(String, Option[(Option[Element], Option[Element])])]] (
            ...
            reverse match { case Some (rev) => Some ((rev._1, None)) case _ => None }, // ObjectReverseModification
            reverse match { case Some (rev) => Some ((rev._2.id, None)) case _ => None }, // PropertiesObject
            forward match { case Some (fwd) => Some ((fwd._1, None)) case _ => None }, // ObjectModification
            forward match { case Some (fwd) => Some ((fwd._2.id, None)) case _ => None } // PropertiesObject
        ).flatten
    }
