...
case class CIMNetworkTopologyProcessor (spark: SparkSession) extends CIMRDD
{
    ...
    def make_graph (): Graph[CIMVertexData, CIMEdgeData] =
    {
        ...
        if (options.debug)
        {
            ...
            val missing = cn.leftOuterJoin (vertices.keyBy (_._2.node_label)).filter (_._2._2 match
            { case None => true
                case _ => false
            }).map (x => (x._2._1, x._1))
            if (!missing.isEmpty)
        ...
        def send_message (triplet: EdgeTriplet[CIMVD, CIMEdgeData]): Iterator[(VertexId, CIMVD)] =
        {
            ...
            else
            {
                ...
                if (triplet.srcAttr.node < triplet.dstAttr.node)
                {
                    ...
                    val voltage = if (null != triplet.attr.voltage) triplet.attr.voltage else
                        if (null != triplet.srcAttr.voltage) triplet.srcAttr.voltage else triplet.dstAttr.voltage
                    ...
                    if (triplet.srcAttr.node > triplet.dstAttr.node)
                    {
                        ...
                        val voltage = if (null != triplet.attr.voltage) triplet.attr.voltage else
                            if (null != triplet.dstAttr.voltage) triplet.dstAttr.voltage else triplet.srcAttr.voltage
