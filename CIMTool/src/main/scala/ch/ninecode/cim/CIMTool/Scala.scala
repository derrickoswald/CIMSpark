    ...
    def valid_attribute_name (attribute: Attribute): String =
    {
        ...
        val stupid_name =
            if (name == attribute.cls.name)
                ...
                if ((attribute.cls.sup != null) && (name == attribute.cls.sup.name))
                    name + "_attr"
                else
                    name
        stupid_name
    ...
    def valid_role_name (role: Role): String =
    {
        ...
        val stupid_name =
            if (name == role.src.name)
                ...
                if ((role.src.sup != null) && (name == role.src.sup.name))
                    name + "_attr"
                else
                    name
        stupid_name
    ...
    def asText (): String =
    {
        ...
        for (cls <- case_classes)
        {
            ...
            if (fields.exists (_.name != sup))
            {
                ...
                s.append (fields.iterator.zipWithIndex.map (x ⇒ (if (x._1.multiple) "emitattrs" else
                    if (x._1.reference) "emitattr" else "emitelem") + " (" + x._2 + ", " + x._1.variable + ")").mkString ("        ", "\n        ", "\n"))
                s.append ("        s.toString\n")
            }
            else
            ...
            s.append (
                if (identified_object)
                    members.iterator.zipWithIndex.map (x ⇒ (x._1, if (x._1.name == "sup") "base" else
                        if (x._1.name == "mRID") "base.id" else (if (x._1.multiple) "masks" else "mask") + " (" + x._1.variable + " (), " + (x._2 - 1) + ")"))
