...
//        new AvroOutputWriterFactory(dataSchema, recordName, recordNamespace)
    ...
    override def buildScan (inputFiles: Array[FileStatus]): RDD[Row] =
    {
        ...
        if (inputFiles.length > 0)
        {
            ...
            sqlContext.createDataFrame (rdd.collect ({ case x: Element if x.getClass () == classOf[ch.ninecode.Connector] => x.asInstanceOf[ch.ninecode.Asset]})).registerTempTable ("Connector")
            sqlContext.createDataFrame (rdd.collect ({ case x: Element if x.getClass () == classOf[ch.ninecode.Junction] => x.asInstanceOf[ch.ninecode.Asset]})).registerTempTable ("Junction")
